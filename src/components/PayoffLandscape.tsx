...
export function PayoffLandscape({
        ...
        >
          <div className="min-w-0">
            ...
            <p className="truncate text-sm text-slate-200">
              {grid.optimalCell
                ? `Best: ${STRATEGY_LABELS[grid.optimalCell.strategyId]} at ${formatCurrency(grid.optimalCell.budget)}/mo`
                : 'Compare every budget and strategy combination'}
            </p>
          </div>
  ...
  return (
    <section className={shell} aria-label="Budget and strategy explorer">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-white/10 px-4 py-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-slate-500">
            A grid of every budget and strategy combo — greener means debt-free sooner. Click a cell to use it.
          </p>
        </div>
        <button
          type="button"
