...
import type { PayoffLandscapePreferences } from './payoffLandscapeTypes';
...
export interface UsePayoffLandscapeResult {
  ...
  setMetric: (metric: PayoffLandscapePreferences['metric']) => Promise<void>;
  setBudgetRange: (min: number, max: number, step: number) => Promise<void>;
}
...
export function usePayoffLandscape(): UsePayoffLandscapeResult {
  ...
  const setMetric = useCallback(
    async (metric: PayoffLandscapePreferences['metric']) => {
  ...
  const setBudgetRange = useCallback(
    async (min: number, max: number, step: number) => {
      await persist({ budgetMin: min, budgetMax: max, budgetStep: step });
    },
    [persist],
  );
