...
import type { ExitAnalysisMode, ExitCompassPreferences } from './exitCompassTypes';
...
function loadLocalPreferences(): ExitCompassPreferences {
  try {
    ...
    const parsed = JSON.parse(raw) as ExitCompassPreferences;
...
export function useExitCompass(): UseExitCompassResult {
  ...
  const setPinnedProperty = useCallback(
    async (property: string | null) => {
      await persist({ pinnedProperty: property });
    },
    [persist],
  );

  const setAnalysisMode = useCallback(
    async (mode: ExitAnalysisMode) => {
  ...
  const setSellAtMonth = useCallback(
    async (month: number) => {
      await persist({ sellAtMonth: month });
    },
    [persist],
  );
  ...
  const setClosingCostPct = useCallback(
    async (pct: number) => {
      await persist({ closingCostPct: pct });
    },
    [persist],
  );
  ...
  const setCapitalGainsRate = useCallback(
    async (rate: number) => {
      await persist({ capitalGainsRate: rate });
    },
    [persist],
  );
  ...
  const setRecaptureRate = useCallback(
    async (rate: number) => {
      await persist({ recaptureRate: rate });
    },
    [persist],
  );
  ...
  const setHoldHorizonMonths = useCallback(
    async (months: number) => {
      await persist({ holdHorizonMonths: months });
    },
    [persist],
  );
  ...
  const setProceedsToDebtPct = useCallback(
    async (pct: number) => {
      await persist({ proceedsToDebtPct: pct });
    },
    [persist],
  );
  ...
  const setShowTaxBreakdown = useCallback(
    async (show: boolean) => {
      await persist({ showTaxBreakdown: show });
    },
    [persist],
  );
