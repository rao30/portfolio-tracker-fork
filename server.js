...
import {
  getPrincipalVelocityPreferences,
...
app.get('/api/principal-velocity', requirePortfolioWebAccess, async (req, res) => {
  if (!requireAuthenticatedUser(req, res)) return;

  try {
    const preferences = await getPrincipalVelocityPreferences(req.supabaseUser.id);
    res.json({ preferences, enabled: isPrincipalVelocityEnabled() });
  } catch (err) {
    console.error('GET /api/principal-velocity', err);
    res.status(500).json({
      error: err instanceof Error ? err.message : 'Failed to load Principal Velocity preferences',
    });
  }
});

app.put('/api/principal-velocity', requirePortfolioWebAccess, async (req, res) => {
  if (!requireAuthenticatedUser(req, res)) return;

  try {
    const preferences = await upsertPrincipalVelocityPreferences(
      req.supabaseUser.id,
      req.body ?? {},
    );
    res.json({ preferences });
  } catch (err) {
    console.error('PUT /api/principal-velocity', err);
    const status = err.status ?? 500;
    res.status(status).json({
