-- Refinance Radar Command Center: per-user market assumptions and UI preferences.
...
CREATE TABLE IF NOT EXISTS public.refinance_radar_preferences (
  ...
  CONSTRAINT refinance_radar_market_rate_valid
    CHECK (market_rate >= 0.01 AND market_rate <= 0.20),
  CONSTRAINT refinance_radar_closing_cost_pct_valid
    CHECK (closing_cost_pct >= 0 AND closing_cost_pct <= 0.10),
  CONSTRAINT refinance_radar_hold_period_valid
    CHECK (hold_period_months >= 12 AND hold_period_months <= 360),
  CONSTRAINT refinance_radar_cash_out_ltv_valid
    CHECK (cash_out_ltv >= 0.50 AND cash_out_ltv <= 0.85),
  CONSTRAINT refinance_radar_min_dscr_valid
    CHECK (min_dscr >= 0.50 AND min_dscr <= 2.0),
  CONSTRAINT refinance_radar_deployment_yield_valid
    CHECK (deployment_yield >= 0 AND deployment_yield <= 0.50),
  CONSTRAINT refinance_radar_refi_term_valid
