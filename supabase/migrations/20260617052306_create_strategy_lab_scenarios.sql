-- Strategy Lab: pinned budget + payoff-strategy scenarios per authenticated user.
...
CREATE POLICY strategy_lab_scenarios_select_own
  ON public.strategy_lab_scenarios FOR SELECT
  USING (user_id = auth.uid());

CREATE POLICY strategy_lab_scenarios_insert_own
  ON public.strategy_lab_scenarios FOR INSERT
  WITH CHECK (user_id = auth.uid());

CREATE POLICY strategy_lab_scenarios_update_own
  ON public.strategy_lab_scenarios FOR UPDATE
  USING (user_id = auth.uid())
  WITH CHECK (user_id = auth.uid());

CREATE POLICY strategy_lab_scenarios_delete_own
  ON public.strategy_lab_scenarios FOR DELETE
  USING (user_id = auth.uid());
