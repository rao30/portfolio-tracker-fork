-- Exit Compass Command Center: per-user preferences for hold/sell/1031 exit analysis.

CREATE TABLE IF NOT EXISTS public.exit_compass_preferences (
  user_id uuid PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
  is_collapsed boolean NOT NULL DEFAULT false,
  pinned_property text,
  analysis_mode text NOT NULL DEFAULT 'all'::text,
  ...
  CONSTRAINT exit_compass_pinned_property_valid
    CHECK (pinned_property IS NULL OR char_length(TRIM(BOTH FROM pinned_property)) > 0),
  CONSTRAINT exit_compass_analysis_mode_valid
    CHECK (analysis_mode = ANY (ARRAY['hold'::text, 'sell'::text, 'exchange'::text, 'all'::text])),
  ...
  CONSTRAINT exit_compass_cap_gains_valid
